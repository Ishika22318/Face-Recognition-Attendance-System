...
class Face_Recognition:
    def __init__(self, root):
        ...
        img_top = img_top.resize((850, 900), Image.ANTIALIAS)
        self.photoimg_top = ImageTk.PhotoImage(img_top)

        f_lbl = Label(self.root, image=self.photoimg_top)
        f_lbl.place(x=0, y=55, width=850, height=900)

        # 2nd image
        img_bottom = Image.open(r"college_images\img4.jpeg")  # Insert Image inside ""
        img_bottom = img_bottom.resize((1070, 900), Image.ANTIALIAS)
        self.photoimg_bottom = ImageTk.PhotoImage(img_bottom)

        f_lbl = Label(self.root, image=self.photoimg_bottom)
        f_lbl.place(x=850, y=55, width=1070, height=900)
